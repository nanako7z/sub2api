...
func TestGetProfileByAccountID(t *testing.T) {
	...
	defaultName := r.GetDefaultProfile().Name
	testCases := []struct {
		accountID    int64
		expectedName string
	}{
...
func TestNewRegistryFromConfig(t *testing.T) {
	...
	if custom1 != nil && custom1.Mode != "npm" {
		t.Errorf("expected custom1 mode npm, got %q", custom1.Mode)
	}
	...
	if custom2 != nil && custom2.Mode != "npm" {
		t.Errorf("expected custom2 mode npm (inherited), got %q", custom2.Mode)
	}
	if custom2 != nil && custom2.ECHPayloadMode != "npm" {
		t.Errorf("expected custom2 payload mode npm (inherited), got %q", custom2.ECHPayloadMode)
	}
	defaultProfile := r.GetDefaultProfile()
	if defaultProfile == nil || defaultProfile.Mode != "npm" {
		t.Errorf("expected default profile mode npm, got %+v", defaultProfile)
	}
	if defaultProfile != nil {
		if defaultProfile.ShapeMode != "npm" {
			t.Errorf("expected default shape mode npm, got %q", defaultProfile.ShapeMode)
		}
