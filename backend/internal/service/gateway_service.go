...
type GatewayCache interface {
	...
	AddStickySessionReverse(ctx context.Context, accountID int64, groupID int64, sessionHash string, ttl time.Duration) error
	// RemoveStickySessionReverse 从反向索引中删除会话绑定
	RemoveStickySessionReverse(ctx context.Context, accountID int64, groupID int64, sessionHash string) error
	// GetStickySessionCounts 批量查询每个账号的活跃粘性会话数
	GetStickySessionCounts(ctx context.Context, accountIDs []int64) (map[int64]int, error)
}
...
// GenerateAffinityHash computes a P2 content-based affinity hash from the request.
// This is used for Claude Code fork session discovery: when a forked session has a new
// session UUID (different P1 hash), the P2 hash lets it find the same account as its parent.
func (s *GatewayService) GenerateAffinityHash(parsed *ParsedRequest) string {
	if parsed == nil {
		return ""
	}
	content := s.extractCacheableContent(parsed)
	if content != "" {
...
// the normal session hash binding. This allows forked Claude Code sessions
// (new session UUID → different P1 hash) to discover the same account via P2 content hash.
func (s *GatewayService) setAffinityBinding(ctx context.Context, groupID *int64, sessionHash string, accountID int64) {
	ah := AffinityHashFromContext(ctx)
	if ah == "" || ah == sessionHash || s.cache == nil {
		return
	}
	_ = s.cache.SetSessionAccountID(ctx, derefGroupID(groupID), ah, accountID, stickySessionTTL)
}

// refreshAffinityTTL refreshes the affinity hash TTL alongside the session TTL.
func (s *GatewayService) refreshAffinityTTL(ctx context.Context, groupID *int64) {
	ah := AffinityHashFromContext(ctx)
	if ah == "" || s.cache == nil {
		return
	}
	_ = s.cache.RefreshSessionTTL(ctx, derefGroupID(groupID), ah, stickySessionTTL)
}
			...
			if len(routingAvailable) > 0 {
				...
				for _, item := range routingAvailable {
					...
					if err == nil && result.Acquired {
						...
						if sessionHash != "" && s.cache != nil {
							s.bindSessionAndAffinity(ctx, groupID, sessionHash, item.account.ID)
						}
						if s.debugModelRoutingEnabled() {
		...
		for len(available) > 0 {
			...
			if err == nil && result.Acquired {
				...
				if !s.checkAndRegisterSession(ctx, selected.account, sessionHash) {
					...
					if sessionHash != "" && s.cache != nil {
						s.bindSessionAndAffinity(ctx, groupID, sessionHash, selected.account.ID)
					}
					return &AccountSelectionResult{
...
func (s *GatewayService) tryAcquireByLegacyOrder(ctx context.Context, candidates []*Account, groupID *int64, sessionHash string, preferOAuth bool) (*AccountSelectionResult, bool) {
	...
	for _, acc := range ordered {
		...
		if err == nil && result.Acquired {
			...
			if sessionHash != "" && s.cache != nil {
				s.bindSessionAndAffinity(ctx, groupID, sessionHash, acc.ID)
			}
			return &AccountSelectionResult{
				Account:     acc,
				Acquired:    true,
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	// 自定义 base URL 模式下，proxy 由 buildCustomRelayURL 通过 query 参数透传。
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		upstreamReq, err := s.buildUpstreamRequest(upstreamCtx, c, account, body, token, tokenType, reqModel, reqStream, shouldMimicHeaders)
		releaseUpstreamCtx()
		if err != nil {
			return nil, err
		}
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.isThinkingBlockSignatureError(respBody) && s.settingService.IsSignatureRectifierEnabled(ctx) {
					...
					retryReq, buildErr := s.buildUpstreamRequest(retryCtx, c, account, filteredBody, token, tokenType, reqModel, reqStream, shouldMimicHeaders)
					releaseRetryCtx()
					if buildErr == nil {
						...
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isThinkingBlockSignatureError(retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									retryReq2, buildErr2 := s.buildUpstreamRequest(retryCtx2, c, account, filteredBody2, token, tokenType, reqModel, reqStream, shouldMimicHeaders)
									releaseRetryCtx2()
									if buildErr2 == nil {
										retryResp2, retryErr2 := s.httpUpstream.DoWithTLS(retryReq2, proxyURL, account.ID, account.Concurrency, s.tlsFPProfileService.ResolveTLSProfile(account))
										if retryErr2 == nil {
											resp = retryResp2
											break
										}
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						budgetRetryReq, buildErr := s.buildUpstreamRequest(budgetRetryCtx, c, account, rectifiedBody, token, tokenType, reqModel, reqStream, shouldMimicHeaders)
						releaseBudgetRetryCtx()
						if buildErr == nil {
							budgetRetryResp, retryErr := s.httpUpstream.DoWithTLS(budgetRetryReq, proxyURL, account.ID, account.Concurrency, s.tlsFPProfileService.ResolveTLSProfile(account))
							if retryErr == nil {
