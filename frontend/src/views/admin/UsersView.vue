          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                ...
                >
                  ${{ (value + (row.gift_balance || 0)).toFixed(2) }}
                </button>
                <!-- Instant tooltip -->
              ...
              <span v-if="row.gift_balance > 0" class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('balance.normal') }}: ${{ value.toFixed(2) }} / {{ t('balance.gift') }}: ${{ row.gift_balance.toFixed(2) }}
              </span>
              <button
...
const DEFAULT_HIDDEN_COLUMNS = ['notes', 'groups', 'subscriptions', 'usage', 'concurrency', 'referrer']
