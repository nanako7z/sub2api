export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      copy: '复制',
      copied: '已复制',
      note: '这些环境变量将在当前终端会话中生效。如需永久配置，请将其添加到 ~/.bashrc、~/.zshrc 或相应配置文件。已包含最小化外联请求配置（禁用非必要流量并关闭 attribution header）。',
      noGroupTitle: '请先分配分组',
      noGroupDescription:
      ...
      antigravity: {
        ...
        geminiCli: 'Gemini CLI',
        claudeNote:
          '这些环境变量将在当前终端会话中生效。如需永久配置，请将其添加到 ~/.bashrc、~/.zshrc 或相应配置文件。已包含最小化外联请求配置（禁用非必要流量并关闭 attribution header）。',
        geminiNote:
